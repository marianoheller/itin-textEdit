...
void TextEdit::setupFileActions()
{
    ...
    a = new QAction(QIcon::fromTheme("document-open", QIcon(rsrcPath + "/fileopen.png")),
                    tr("&Open CSV..."), this);
    //a->setShortcut(QKeySequence::Open);
    connect(a, SIGNAL(triggered()), this, SLOT(CSVOpen()));
    tb->addAction(a);
    menu->addAction(a);
...
bool TextEdit::parseLoadCSV(const QString &f)
{
    if (!QFile::exists(f))
        return false;
    QFile file(f);
    ...
    for ( int i=0 ; i<csvRawData.size() ; i++)
    {
        ...
        if ( csvRawData.at(i).at(TABLA_DATCOL_1) == "FAME por IR")
        {
            QString aux = csvRawData.at(i).at(TABLA_DATCOL_4);
            aux = aux.replace("10","15");
            csvRawData[i].replace(TABLA_DATCOL_4,aux);
        }
    }
    ...
    QByteArray data;
//    for (QStringList auxRowData : csvRawData)
//    {
//        for (QString auxString : auxRowData)
//        {
//            int i;
//            while( (i=auxString.indexOf("\r")) != -1)
//                auxString.remove(i,1);
//            auxString.append("\n");
//            data.append(auxString.toLocal8Bit());
//        }
//    }
//    data.append(htmlData.toLocal8Bit());
...
QString TextEdit::transformCSVtoHTML(QVector<QStringList> * data)
{
...
//    QTextDocument* html = new QTextDocument(textEdit->toHtml());
//    bool success = writer.write(html);
